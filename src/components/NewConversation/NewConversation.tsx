...
export default function NewConversation() {
  ...
  const [submissionMode, setSubmissionMode] =
    useState<string>("uploadRecording"); // to hide or show the live recorder
  ...
  async function submitJob(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      updateProgressBar({
        ...
        additionalInfo: `Error uploading ${filePath!.name}: ${
          (e as Error).message
        }`,
      });
      setIsSubmitting(false);
    ...
    try {
      ...
      if (startJob?.MedicalScribeJob?.MedicalScribeJobStatus) {
        updateProgressBar({
          ...
          additionalInfo: `Audio file successfully uploaded to S3 and submitted to HealthScribe at ${dayjs(
            ...
            "MM/DD/YYYY hh:mm A"
          )}. Redirecting to conversation list in 5 seconds.`,
        });
        await sleep(5000);
        ...
        updateProgressBar({
          ...
          additionalInfo: `Response from HealthScribe: ${JSON.stringify(
            startJob
          )}`,
        });
      }
      ...
      updateProgressBar({
        ...
        additionalInfo: `Error submitting job to HealthScribe: ${
          (e as Error).message
        }`,
      });
      setIsSubmitting(false);
  ...
  return (
    <ContentLayout
      header={
        ...
        >
          <h2 style={{ color: "#000000" }}> New Conversation </h2>
        </Header>
      }
    >
                  ...
                  <SpaceBetween direction="vertical" size="xl">
                    <div className={styles.submissionModeRadio}>
                      <RadioGroup
                        ...
                        onChange={({ detail }) =>
                          setSubmissionMode(detail.value)
                        }
