...
import {
  ...
  deleteHealthScribeJob,
} from "@/utils/HealthScribeApi";
  ...
  return (
        ...
        <Box float="right">
          <SpaceBetween direction="horizontal" size="xs">
            ...
            <Button
              ...
              onClick={() =>
                doDelete(
                  selectedHealthScribeJob?.[0]?.MedicalScribeJobName || ""
                )
              }
            >
    ...
    >
      ...
      <Alert statusIconAriaLabel="Info">
        Proceeding with this action will delete the conversation but not the
        associated data (audio file, results JSON) from S3.
      </Alert>
    </Modal>
  );
}
...
type TableHeaderActionsProps = {
  setSearchParams: React.Dispatch<
    React.SetStateAction<ListHealthScribeJobsProps>
  >;
  selectedHealthScribeJob: MedicalScribeJobSummary[];
  setDeleteModalActive: React.Dispatch<React.SetStateAction<boolean>>;
  refreshTable: () => void;
  ...
  const actionButtonDisabled = useMemo(
    () =>
      ...
      !["COMPLETED", "FAILED"].includes(
        selectedHealthScribeJob[0].MedicalScribeJobStatus || ""
      ),
    [selectedHealthScribeJob]
  );
      ...
      <Form>
        <Grid gridDefinition={[{ colspan: 5 }, { colspan: 3 }]}>
          <Input
            ...
            onChange={({ detail }) =>
              handleInputChange("JobNameContains", detail.value)
            }
          />
          <Select
            selectedOption={
              statusSelections.find((s) => s.value === searchParams?.Status) ||
              null
            }
            onChange={({ detail }) =>
